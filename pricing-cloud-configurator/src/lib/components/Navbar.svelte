<script lang="ts">
	import { Cloud, Menu, X } from '@lucide/svelte';
...
<header class="sticky top-0 z-50 border-b border-surface-200-800 bg-surface-50-950/85 backdrop-blur">
	<nav class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
		<a href="#top" class="flex items-center gap-2 font-bold tracking-tight" onclick={close}>
			...
			<span class="text-lg">{provider.name}</span>
		</a>
		...
		<div class="flex items-center gap-2">
			<a href="#konfigurator" class="btn hidden preset-filled-primary-500 sm:inline-flex">
				Policz serwer
			</a>
			<button
